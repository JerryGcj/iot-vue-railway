  ...
  export default {
    ...
    methods: {
          ...
          if (!err) {
            ...
            this.$refs.editableTable1.getValues((error, values) => {
              ...
              if (error === 0) {
                ...
                let httpurl = '/telecomrefcardcost/telecomRefCardCost/updateAndAdd';
                let method = 'post';
      ...
      queryDiscountPackage(){
        ...
        var that = this;
        queryDiscountPackage({operatorType:"3"}).then((res)=>{
          if(res.success){
          that.packageData = [];
          let treeList = res.result
          for(let a=0;a<treeList.length;a++){
        ...
        handleDelete(props) {
          ...
          for (let i = 0; i < packageValue.length; i++) {
              if(rowId == packageValue[i].id){
                let httpurl = '/telecomrefcardcost/telecomRefCardCost/delete';

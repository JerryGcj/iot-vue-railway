<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :md="4" :sm="6">
            <a-form-item label="运营商">
              <j-dict-select-tag v-model="queryParam.operatorId" placeholder="请选择运营商" dict-code="iot_operator,operator_name,id,operator_type_id=3"></j-dict-select-tag>
            </a-form-item>
          </a-col>

          <a-col :md="4" :sm="6">
            <a-form-item label="卡状态">
              <j-dict-select-tag v-model="queryParam.simStatus" placeholder="请选择" dictCode="telecom_status"/>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="8">
    ...
    <div class="table-operator">
      ...
      <a-button type="primary" v-has="'telecom:importExcel'" :action="importExcelUrl" icon="import" @click="handleCardImport">卡导入</a-button>
      <a-button type="primary" v-has="'telecom:CardDistribution'" icon="" @click="handleCardDistribution"  @hello="onClearSelected">分配</a-button>
      <a-button type="primary" v-has="'telecom:CardStateChange'" icon="" @click="handleCardStateChange">状态变更</a-button>
      <a-button type="primary" v-has="'telecom:handleCorrection'" icon="" @click="handleCorrection">同步用量</a-button>
      <a-button v-has="'telecom:setTime'" type="primary" icon="" @click="setTime">设置激活及到期时间</a-button>
      <!--<a-button type="primary" v-has="'cardInformationList:DataUpdate'" icon="" @click="handleDataUpdate">数据更新</a-button>-->
      <a-button type="primary" v-has="'telecom:CardFlowUsageRatio'" icon="" @click="handleCardFlowUsageRatio">设置用量比例</a-button>
      <a-button v-has="'telecom:stateChange'" type="primary" icon="" @click="syncCardStatus" @hello="onClearSelected">用量同步更改</a-button>
      <a-button v-has="'telecom:monitoring'" type="primary" icon="" @click="handleMonitorUse" @hello="onClearSelected">用量监控更改</a-button>
      <a-button type="primary" icon="" @click="renewal">续费</a-button>
      <a-button v-has="'telecom:setMony'" type="primary" icon="" @click="modifyPrice" @hello="onClearSelected">改价</a-button>
      <a-button type="primary" v-has="'telecom:printQRcode'" icon=""  @click="printQRcode" @hello="onClearSelected">打印二维码</a-button>
      ...
      <a-button type="primary" icon="download" @click="handleExport('电信卡信息表')">导出</a-button>
      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a-popconfirm title="确定删除吗?" @confirm="() => handleDelete(record.id)">
            <a v-has="'telecom:CardDelete'">删除</a>
          </a-popconfirm>
    ...
    <card-information-modify-price ref="CardInformationModifyPrice" @ok="modalFormOk"></card-information-modify-price>
    <card-information-sync-status ref="CardInformationSyncStatus" @ok="modalFormOk"></card-information-sync-status>
    <card-information-export ref="CardInformationExport" @ok="modalFormOk"></card-information-export>
    <card-information-monitor-use ref="CardInformationMonitorUse" @ok="modalFormOk"></card-information-monitor-use>
  ...
  computed: {
    ...
    methods: {
      ...
      handleCardDistribution: function () {
        ...
        var params = this.getQueryParams();//查询条件
        if((null==this.queryParam.customerId||'undefined'==this.queryParam.customerId||""==this.queryParam.customerId)&&
          (null==this.queryParam.iccidStart||'undefined'==this.queryParam.iccidStart||""==this.queryParam.iccidStart)&&
          (null==this.queryParam.iccidEnd||'undefined'==this.queryParam.iccidEnd||""==this.queryParam.iccidEnd)&&
          (null==this.queryParam.customIccidStart||'undefined'==this.queryParam.customIccidStart||""==this.queryParam.customIccidStart)&&
          (null==this.queryParam.customIccidEnd||'undefined'==this.queryParam.customIccidEnd||""==this.queryParam.customIccidEnd)&&
          (null==this.queryParam.accessNumber||'undefined'==this.queryParam.accessNumber||""==this.queryParam.accessNumber)&&
          (null==this.queryParam.cardState||'undefined'==this.queryParam.cardState||""==this.queryParam.cardState)){
      ...
      printQRcode: function () {
        if(this.selectedRowKeys.length>0){
          ...
          getAction('/telecomcardinformation/telecomCardInformation/getQRcodeById', {ids:ids}).then((res) => {
            console.log(res)
            if (res.success) {
        ...
        var params = this.getQueryParams();//查询条件
        console.log(params)
        getAction('/telecomcardinformation/telecomCardInformation/getQRcode', params).then((res) => {
          console.log(res)
          if (res.success) {
